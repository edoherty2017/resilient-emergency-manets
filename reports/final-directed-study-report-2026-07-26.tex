...
2026-07-19 are git-timestamped by commit \code{1229309} (branch
\code{trial2-field-campaign-2026-07}); every SHA-256 digest cited in this
report was re-verified against its artifact on 2026-07-26.
...
``Any headline claim tied either to the corrected simulation runs or to the Trial 2 measurements. Superseded numbers clearly archived and not mixed with final results.'' & \S\ref{sec:repro} data statement; \S\ref{sec:yearscale}--\ref{sec:insight} trace to release\_v1; model-to-model screens labeled (\S\ref{sec:trial1}); superseded outputs archived separately & Satisfied \\
...
Final package: frozen protocol pack $\cdot$ raw dataset $\cdot$ scored results $\cdot$ corrected simulation package $\cdot$ full written report & Appendix~\ref{app:packs} $\cdot$ raw pulls $+$ dataset evidence release \code{evidence-2026-07-26} (128{,}721 obs, 0 calibration-grade, \code{MANIFEST.json}) $\cdot$ two-tier scoring $+$ Trial 1 gates $\cdot$ release\_v1 $\cdot$ this report & All five items delivered (item 2 as a zero-calibration-grade evidence release; item 3 as the two-tier contact scoring --- the RMSE/heatmap validation deliverable remains deferred per \S\ref{sec:objectives}) \\
\bottomrule
\end{tabularx}
\end{table}
