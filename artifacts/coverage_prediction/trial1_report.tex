...
network on Mt.\ Washington, NH. The primary objective was to characterize LoRa signal
propagation along a full 10-hour ascent/descent route and assess the feasibility of a
...
18:33 & Collector stopped. 686 total packets logged across 6\,h 9\,m of active collection. \\
...
\midrule
Total packets logged          & 686 \\
Active collection window      & 12:24 -- 18:33 EDT (6\,h 9\,m) \\
Unique source nodes heard     & 50 \\
...
5-min windows with $\geq$1 packet received (descent) & \textcolor{okgreen}{\textbf{100\%}} \\
\bottomrule
\end{tabular}
\end{center}

In every 5-minute window of the 6\,h\,9\,m descent, the HEAD node demodulated at least one
Meshtastic packet from \emph{some} source --- any of $\sim$50 ambient mesh nodes at unknown
...
library-level reconnection behavior, not a hardware failure. The gap covered the entire
Ammonoosuc Ravine ascent above treeline --- the highest-risk terrain segment.
...
during the trial, limiting the ability to compute precise source-to-receiver distances
for observed packets.
...
% ─────────────────────────────────────────────────────────────────
\section{Proposed Relay Infrastructure}

\subsection{Node Placement}

Three fixed nodes are proposed at GPS-derived locations corresponding to the treeline
crossings on each trail. Each relay sits at the terrain transition where it has
simultaneous line-of-sight both uphill (toward summit) and downhill (toward trailhead),
maximizing coverage with a single node.
...
\subsection{Predicted Link Performance}
...
All predicted links clear the receiver sensitivity floor ($-131$\,dBm) by more than
31\,dB. Applying the shadowing standard deviation of $\sigma \approx 7$\,dB from
\cite{bianco2021}, the worst-case link (Ammo $\to$ Gateway) retains a margin of
$31.2 - 3\sigma = 10.2$\,dB at the 99.87th percentile of shadowing.
...
a planning threshold is used.) Under this model the 2\,h 48\,m collector gap would have
been covered: the Ammo Relay at 1.68\,km from the summit lies well within
$d_{\max}^{\,\text{alpine}}$, with a predicted link margin of 58.1\,dB --- subject to
the terrain-profile caveat above, since parts of the gap segment are inside the ravine
and may be non-line-of-sight to the relay.

\subsection{Automation Chain}

With the proposed infrastructure, an SOS event follows the chain:
\[
...
Target end-to-end latency: $<$30\,seconds. Total one-time hardware cost: $\approx\$690$
(two relay nodes at $\sim\$120$ each, gateway Pi + radio $\sim\$150$, Starlink Mini
$\sim\$300$). Where AMC huts already provide Starlink connectivity at the trailhead,
new hardware cost reduces to $\approx\$240$.
...
physically plausible (LoRa SF11 demodulates at these levels, and packets were in fact
received in forested terrain during the descent) but with zero fade margin and
...
Two Heltec V3 radios will be connected to the Pi simultaneously:
\begin{itemize}[noitemsep]
  \item \textbf{Radio A}: restricted to \texttt{maxHops=1} --- captures direct links only
  \item \textbf{Radio B}: unrestricted (\texttt{maxHops=3}) --- captures all relayed packets
\end{itemize}
Packets appearing on Radio B but not Radio A are confirmed mesh hops.
No inference or FSPL estimation required.
...
Contact Giulio Maria Bianco and Abraham Mejia-Aguilar at EURAC Research (Bolzano, Italy)
during planned visit in July 2026. Their published path loss measurements in Alpine
terrain \cite{bianco2021, bianco2020} provide direct empirical grounding for the
terrain-augmented model used here.
...
Trial~2 deploys \texttt{meshnode1} as an independent hiker node on a separate trail
at a known GPS-tracked distance from the HEAD. Combined with the dual-radio
hop-count setup (Section~\ref{sec:forward}), every observation will be a verified
direct link with GPS on both ends --- the minimal requirement for valid calibration.
...
\textit{IEEE International Conference on Wireless and Mobile Computing, Networking and
Communications (WiMob)}, 2020.
DOI: 10.1109/WiMob50308.2020.9243817

\bibitem{review2024}
A.\ Ferreira et al.,
``A Critical Review of the Propagation Models Employed in LoRa Systems,''
\textit{MDPI Sensors}, 2024.
\url{https://pmc.ncbi.nlm.nih.gov/articles/PMC11207269/}

\bibitem{pdr2021}
``Experimental Evaluation of the Packet Reception Performance of LoRa,''
\textit{MDPI Sensors}, 2021.
\url{https://pmc.ncbi.nlm.nih.gov/articles/PMC7916015/}

\bibitem{itmlogic2020}
E.~Oughton, T.~Russell, J.~Johnson, C.~Yardim, and J.~Johnson,
