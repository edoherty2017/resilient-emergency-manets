...
6. Starlink Mini WAN path up (for Hermes reach-back over Tailscale)
...
- [ ] GPS capture working
- [ ] Starlink Mini reachable from meshhead and `ssh meshradiohead` works from doher/Hermes side
...
## D) Control-plane fallback mode (expected during range loss)
When hiker nodes move out of Starlink/IP reach:
- [ ] mark `CONTROL_PLANE_DOWN_START`
- [ ] keep meshhead+Starlink online in your pack
- [ ] continue low-rate Meshtastic command/heartbeat attempts to remote nodes
- [ ] log ACK/no-ACK and latency class
- [ ] mark `CONTROL_PLANE_DOWN_END` when SSH/Tailscale recovers
...
Also mark control-plane outage windows so later analysis does not confuse IP backhaul loss with RF mesh failure.
