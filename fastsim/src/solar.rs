...
pub fn solar_position(lat_deg: f64, lon_deg: f64, day_of_year: u32, sec_of_day: f64)
    -> (f64, f64)
{
    let hour = sec_of_day / 3600.0;
    let frac_year = 2.0 * std::f64::consts::PI / 365.0
        * (day_of_year as f64 - 1.0 + (hour - 12.0) / 24.0);
    let decl = 0.006918 - 0.399912 * frac_year.cos() + 0.070257 * frac_year.sin()
        - 0.006758 * (2.0 * frac_year).cos() + 0.000907 * (2.0 * frac_year).sin()
        - 0.002697 * (3.0 * frac_year).cos() + 0.00148 * (3.0 * frac_year).sin();
    let eqtime_min = 229.18 * (0.000075 + 0.001868 * frac_year.cos()
...
pub fn solar_power_w(
    lat: f64, lon: f64, day_of_year: u32, sec_of_day: f64, kt: f64,
    horizon_deg: &[f64], site: &SiteSolar, panel_w: f64, system_eff: f64,
) -> f64 {
    ...
    let plane = if site.pyramid {
        ...
        for face_az in [0.0f64, 90.0, 180.0, 270.0] {
            ...
            p += 0.25 * (dni * cos_inc.max(0.0)
                + diffuse_h * (1.0 + beta.cos()) / 2.0);
        }
        p
    } else {
        direct_h + diffuse_h
